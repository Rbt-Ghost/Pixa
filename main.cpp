...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        if (negativeButton.getIsPressedInside() && inputTexture.getSize() != sf::Vector2u(0,0)) {
        ...
        window.draw(redFilterButton);
        window.draw(greenFilterButton);
        window.draw(blueFilterButton);
        window.draw(mosaicButton);
        window.draw(smoothingButton);
